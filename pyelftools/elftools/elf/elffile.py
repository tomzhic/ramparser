...
class ELFFile(object):
    ...
    def get_section_by_name(self, name, start_section_name=None):
        ...
        if not self._section_name_map:
            if start_section_name == None:
                ready = True
            else:
                ready = False
            for i, sec in enumerate(self.iter_sections()):
                if ready:
                    if sec.name not in self._section_name_map:
    ...
    def get_dwarf_info(self, relocate_dwarf_sections=True,
        ...
        for secname in (b'.debug_info', b'.debug_abbrev', b'.debug_str',
            ...
            section = self.get_section_by_name(secname, start_section_name)
            if section is None:
        ...
        return DWARFInfo(
                ...
                debug_macinfo_sec=debug_sections[b'.debug_macinfo'])
