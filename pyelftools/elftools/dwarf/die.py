...
from .enums import ENUM_DW_MACINFO, ENUM_DW_FORM
from .macroinfo import MacroInfo
...
class DIE(object):
    ...
    def __init__(self, cu, stream, offset):
        ...
        self.stream = stream
        self.offset = offset
        self.cu_offset = self.offset - self.cu.cu_offset
    ...
    def _parse_DIE(self):
        ...
        for name, form in abbrev_decl.iter_attr_specs():
            ...
            value = self._translate_attr_value(form, raw_value,structs)
            
            # Parses through macro info
            # Use value.defines, value.undefines to access macro definitions
            # and undefinitions
            if name == "DW_AT_macro_info":
                macro_info = MacroInfo(self.dwarfinfo.debug_macinfo_sec.stream, value, structs)
                value = macro_info
    ...
    def _translate_attr_value(self, form, raw_value,structs):
        ...
        if form == 'DW_FORM_strp':
            ...
            return self._translate_attr_value(form, raw_value,structs)
        else:
            value = raw_value
        return value
